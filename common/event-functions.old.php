...
function getLocationLineEvents($type, $location)
{
	...
	switch ($type)
	{
		...
		case 'Track Amplified':
			$sqlBit = $sqlBetween." AND ((tracks > 1 AND safeworking_why != 'opened') OR safeworking_why = 'singled' ) ";
			//$sqlBit = $sqlBetween." AND tracks > '1' ";
			break;
		case 'Safeworking':
			...
			break;
		case 'Line Closed':
			$sqlBit = $sqlBetween." AND tracks = '0' ";
			break;
		...
		case 'By Date':
			$sqlBit = " AND (( safeworking_why != 'singled' AND (safeworking_why = 'opened') ) 
				...
				OR ( description != '' AND start_location = '' AND end_location = '' )) ";
			break;
		default:
			$sqlBit = " AND description != '' AND start_location = '' AND end_location = '' ";	
			break;			
	}
	...
	if ($type != 'By Date')
	{
		$sql = "SELECT DATE_FORMAT(date, '".DATE_FORMAT."') AS fdate, tracks, safeworking, 
		gauge, start_location, end_location, date, line, description, 
		...
		WHERE $lineSQL $sqlBit AND display != 'hide' 
		AND start_location = STARTKM.location_id AND end_location = ENDKM.location_id 
		ORDER BY date ASC";
	}
	else
	{
		$sql = "SELECT DATE_FORMAT(date, '".DATE_FORMAT."') AS fdate, date AS plaindate, tracks, safeworking, 
		gauge, start_location, STARTKM.km AS start_distance, end_location, 
		ENDKM.km AS end_distance, date, line, description, 
		safeworking_middle, '-', safeworking_why, source, sourcedetail, dateAccuracy   
		FROM railline_events, locations_raillines STARTKM, locations_raillines ENDKM 
		WHERE $lineSQL AND (( safeworking_why != 'singled' 
		AND (safeworking_why = 'opened') ) OR ( ((tracks > '1' AND safeworking_why != 'opened') 
		OR safeworking_why = 'singled' ) ) OR ( safeworking != '' ) OR ( tracks = '0' ) 
		OR ( gauge != 'BG' ) OR ( description != '' AND start_location = '' AND end_location = '' ))  
		AND display != 'hide'
		AND start_location = STARTKM.location_id AND end_location = ENDKM.location_id 
		UNION
		...
		WHERE display != 'tracks' AND lr.location_id = l.location_id 
		AND type = type_id AND (".isImportantLocation().") AND $lineSQL AND name != '' 
	...
	if ($numberOfRows > 0 AND $sqlBit != '' )
	{
		...
		{	
			...
			{
				case '0':
					$details = 'Line closed '.$start.' to '.$end;
					break;
				case '1':
					$details = 'Line opened '.$start.' to '.$end;
					break;
				case '2':
					$details = 'Line duplicated '.$start.' to '.$end;
					break;
				case '3':
					$details = 'Line triplicated '.$start.' to '.$end;
					break;
				case '4':
					$details = 'Line quadruplicated '.$start.' to '.$end;
					break;
				case '6':
					$details = 'Line amplified to six tracks '.$start.' to '.$end;
					break;	
				case 'opened':
			...
			{
				case 'singled':
					$details = 'Line singled '.$start.' to '.$end;	
					break;
				case 'opened':
					$details = 'Line opened '.$start.' to '.$end;	
					break;	
			...
			{
				case 'dg':
					$details = 'Converted to Dual Gauge between '.$start.' and '.$end;	
					break;
				case 'sg':
					$details = 'Converted to Standard Gauge between '.$start.' and '.$end;	
					break;	
				case 'ng':
					$details = 'Converted to Narrow Gauge between '.$start.' and '.$end;	
					break;
			}
			...
			elseif ($type == 'Safeworking' OR ($type == 'By Date' AND $safeworking != '' AND $safeworking != '-' AND $date != $pastdate) )
			{
				...
				if ($locationID != "" AND $middleID != '0')	// for specific locations AND single section split into two
				{
					...
					{
						$details = $middle.' closed. '.$nicename.' section now '.$start.' - '.$end;$status = ' closed. ';
					}
					elseif ($locationID == $middleID)
					{
						$details = $middle.' opened. '.$nicename.' section now  '.$start.' -  '.$middle.' - '.$end;			
					}
					else
					{
						...
						{
							$details = $middle.' opened. '.$nicename.' section now '.$start.' - '.$middle;
						}
						else
						{
							$details = $middle.' opened. '.$nicename.' section now '.$middle.' - '.$end;
						}
					}
				}
				...
				else
				{
					...
					if ($middleID != '0')	
					{
						...
						{
							$details = $middle.' opened in '.$nicename.' section '.$start.' - '.$end;
						}
					}
					// for yard limits
					else if ($nicename == 'Yard Limits')
					{
						...
						{
							$details = 'Main line '.$start.' - '.$end.' downgraded to siding';
						}
						else
						{
							$details = '';
					...
					{
						$details = $nicename.' provided '.$start.' - '.$end;
					}
				}
			}
