...
function getLineLocationEvents($lineId, $type)	
{
	...
	if ($type == 'Location Opened')
	{
		...
		$sql = "SELECT DATE_FORMAT(open, '".DATE_FORMAT."')  AS fdate, openAccuracy AS dateaccuracy, 
			...
			AND type = type_id AND (".IMPORTANT_LOCATION.") 
	...
	elseif ($type == 'Location Closed')
	{
		...
		$sql = "SELECT DATE_FORMAT(close, '".DATE_FORMAT."') AS fdate, closeAccuracy AS dateaccuracy, 
			...
			AND type = type_id AND (".IMPORTANT_LOCATION.") AND line_id = $lineId 
...
function getLocationLineEvents($type, $location)
{
	...
	else
	{
		...
		AND RE.display != 'hide'
		UNION
		SELECT event_id, DATE_FORMAT(l.open, '".DATE_FORMAT."') AS fdate, l.open AS plaindate, 'opened' AS tracks, 
		'-', '-', '-', '-', 
		...
		WHERE display != 'tracks' AND lr.location_id = l.location_id 
		AND type = type_id AND (".IMPORTANT_LOCATION.") AND line_id = '$lineId' AND name != '' 
		AND open != '".DATE_UNKNOWN_OPEN."' AND open != '".DATE_UNKNOWN_CLOSE."' 
		UNION
		SELECT event_id, DATE_FORMAT(l.close, '".DATE_FORMAT."')  AS fdate, l.close AS plaindate, 'closed' AS tracks, 
		'-', '-', '-', '-', 
	...
	if ($numberOfRows > 0 AND $sqlBit != '' )
	{
		...
		{
			$eventId = MYSQL_RESULT($result,$i+$extraEvents,"event_id");
			...
			switch ($tracks)
			{
				...
				case '6':
					...
					break;	
				case 'opened':
					$details = $eventLocationName.' '.strtolower($safeworkingType).' opened';
					$details = eregi_replace('industry', '(industry)', $details);
					break;
				case 'closed':
					$details = $eventLocationName.' '.strtolower($safeworkingType).' closed';
					$details = eregi_replace('industry', '(industry)', $details);
					break;
			}
			...
			if ($type == 'Line Opened')
			{
				...
				while ($extraEvents < ($numberOfRows-$i))
				{
					if ($plainDate == MYSQL_RESULT($result,$i+$extraEvents,"date")
							AND MYSQL_RESULT($result,$i+$extraEvents,"safeworking_why") == 'opened'
							AND $eventId != MYSQL_RESULT($result,$i+$extraEvents,"event_id"))
					{
						echo 'start'.MYSQL_RESULT($result,$i+$extraEvents,"start_name").'<br>';
						echo 'end'.MYSQL_RESULT($result,$i+$extraEvents,"end_name").'<br>';
						$extraEvents++;
					}
					else
					{
						echo 'start'.MYSQL_RESULT($result,$i+$extraEvents,"start_name").'<br>';
						echo 'end'.MYSQL_RESULT($result,$i+$extraEvents,"end_name").'<br><br>';
						break;
					}
				}
				...
				echo 'extraEvents > '.$extraEvents.'<br>plainDate > '.$date."<br>".$foo."<br>".$sameDate."<br>";
				
				
				if ($sameDate > '1' AND !$tram AND $locationKm != '')	// chains together events of the same date
				{
					$details = 'Line opened '.$startLocationName;
					$sqlmultilocations = "SELECT ENDLOCATION.name as end_name , STARTKM.km AS start_distance, ENDKM.km AS end_distance 
						FROM railline_events
						INNER JOIN locations_raillines STARTKM ON start_location = STARTKM.location_id
						INNER JOIN locations STARTLOCATION ON start_location = STARTLOCATION.location_id 
						INNER JOIN locations_raillines ENDKM ON end_location = ENDKM.location_id 
					...
					{
						echo "loop > ".MYSQL_RESULT($result,$i+$x,"date")."<hr>";
						$middleName = MYSQL_RESULT($multiLocations,$x,"end_name");
						$details .= " - $middleName";
					}
					$i = $i + ($sameDate - 1);
					$pastdate = $date;
			...
			if ($type == 'Track Amplified')
			{
				...
				if ($sameDate > '1' AND !$tram)	// chains together events of the same date
				{
					...
					$sqlmultilocations = "SELECT STARTLOCATION.name AS start_name, ENDLOCATION.name AS end_name,
						STARTKM.km AS start_distance, ENDKM.km AS end_distance, MIDDLELOCATION.name AS middle_name, 
						start_location, end_location, safeworking, RE.tracks
						FROM railline_events RE
						INNER JOIN locations_raillines STARTKM ON start_location = STARTKM.location_id
						INNER JOIN locations STARTLOCATION ON start_location = STARTLOCATION.location_id 
						INNER JOIN locations_raillines ENDKM ON end_location = ENDKM.location_id 
					...
					if ($sameDate > 1 AND !$tram)
					{
						...
						$starttracks = MYSQL_RESULT($multiLocations,'0',"RE.tracks");
						$startsafe = MYSQL_RESULT($multiLocations,'0',"safeworking");
						$subdetails = split(' ', $details);
						$details = 'Line '.$subdetails[1].' '.stripslashes(MYSQL_RESULT($multiLocations,'0',"start_name"));
						
						for ($x = 0; $x < $sameDate; $x++)
						{
							$middletracks = MYSQL_RESULT($multiLocations,$x,"RE.tracks");
							$middlesafe = MYSQL_RESULT($multiLocations,$x,"safeworking");
							
							if($middletracks == $starttracks AND $middlesafe == $startsafe)
							{
								$details = $details.' - '.stripslashes(MYSQL_RESULT($multiLocations,$x,"end_name"));
							}
							else
							{
								$sameDate = 1;
			...
			{
				$sameDateResult = MYSQL_QUERY("SELECT count(event_id) 
					FROM railline_events 
					WHERE line = '".$lineId."' AND date = '".$plainDate."' 
					AND (safeworking_why = 'replaced' OR safeworking_why = 'opened') 
					AND safeworking != ''", locationDBconnect());
				$sameDate = MYSQL_RESULT($sameDateResult,0 , "count(event_id)");
				$nicename = stripslashes(MYSQL_RESULT($result,$i,"safeworking_type"));
				
				// for specific locations AND single section split into two
				if ($locationID != "" AND $middleID != '0')	
				{
				...
				else
				{
					...
					if ($middleID != '0')	
					{
						...
						if ($safeworkingType == 'closed')
						{
							$details = "$middleName closed. $nicename section now $startLocationName - $endLocationName";
						}
						else
						{
							$details = "$middleName opened in $nicename section $startLocationName - $endLocationName";
						}
					}
					// for yard limits
...
function getLocationEvents($location)
{
	...
	else if ($openPlain != DATE_UNKNOWN_OPEN AND !$isCrossing)	
	{	
		$locationevents[0]["date"] = formatDate($location['open'], $location['approxOpen']);
		$locationevents[0]["details"] = 'Opened';
		...
		{ 
			$locationevents[1]["date"] = formatDate($location['close'], $location['approxClose']);
			$locationevents[1]["details"] = 'Closed';
		}
	}
