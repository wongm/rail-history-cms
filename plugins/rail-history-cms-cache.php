...
$plugin_description = gettext("Adds static HTML cache functionality to Zenphoto, with enhancements for Rail History CMS custom pages.");
$plugin_author = "Malte Müller (acrylian), Stephen Billard (sbillard)";

$option_interface = 'static_html_cache_railHistoryCMS';
...
$_zp_HTML_cache = new static_html_cache_railHistoryCMS();
if (isset($_zp_HTML_cache) && $_zp_HTML_cache) {
	$_zp_HTML_cache->startHTMLCache();
}

class static_html_cache_railHistoryCMS {
			...
			if (!empty($cachefilepath)) {
				...
				if(file_exists($cachefilepath)) {
					...
					if (!isset($_POST['comment']) && time()-$lastmodified < getOption("static_cache_expire")) {
						...
						echo "<!-- ".sprintf(gettext('Cached content of %3$s served by static_html_cache_railHistoryCMS in %1$.4f seconds plus %2$.4f seconds unavoidable Zenphoto overhead.'),$end-$start_cache,$start_cache-$start,date('D, d M Y H:i:s',filemtime($cachefilepath)))." -->\n";
						exitZP();
					}
				}
	...
	function static_html_cache_railHistoryCMS_options() {
