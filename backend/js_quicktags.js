...
function edToolbar() {
	...
	if (edShowExtraCookie()) {
		document.write(
			...
			+ */'<input type="button" id="ed_extra_show" class="ed_button" onclick="edShowExtra()" value="»" style="visibility: hidden;" />'
			+ '</span><br />'
			+ '<span id="ed_extra_buttons">'
			+ '<input type="button" id="ed_extra_hide" class="ed_button" onclick="edHideExtra();" value="«" />'
		);
	}
	else {
		document.write(
...
function edInsertExtLink(myField, i, defaultValue) {
	...
	if (!edCheckOpenTags(i)) {
		var URL = prompt('Enter the gallery link' ,defaultValue);
		if (URL) {
			edButtons[i].tagStart = '[[Image:' + URL + '|thumb|';
			edInsertTag(myField, i);
		}
	}
	else {
